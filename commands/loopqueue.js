...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (player.queueRepeat) {
      ...
      client.sendTime(message.channel, `:repeat: Повтор очереди \`Выключено\``);
    } else {
      player.setQueueRepeat(true);
      client.sendTime(message.channel, `:repeat: Повтор очереди \`Включено\``);
    }
  },
  SlashCommand: {
    /**
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (player.queueRepeat) {
        player.setQueueRepeat(false);
        client.sendTime(interaction, `:repeat: **Повтор очереди** \`Выключено\``);
      } else {
        player.setQueueRepeat(true);
        client.sendTime(interaction, `:repeat: **Повтор очереди** \`Включено\``);
      }
