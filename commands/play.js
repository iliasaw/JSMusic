...
module.exports = {
  name: "play",
  description: "Играйте свои любимые песни",
  usage: "[song]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Используйте - **\`${GuildDB.prefix}play [song]\``
      );
      ...
      if (SearchString.match(client.Lavasfy.spotifyPattern)) {
        ...
        if (Searched.loadType === "PLAYLIST_LOADED") {
          ...
          SongAddedEmbed.setAuthor(
            `Список видео добавлен в очередь`,
          ...
          if (!player.playing && !player.paused && !player.queue.size)
            player.play();
          SongAddedEmbed.setAuthor(`Добавлено к очереди`, client.botconfig.IconURL);
      ...
      if (search.match(client.Lavasfy.spotifyPattern)) {
        ...
        switch (Searched.loadType) {
          ...
          case "SEARCH_RESULT":
            ...
            SongAdded.setAuthor(`Добавлено к очереди`, client.botconfig.IconURL);
        ...
        try {
          ...
          return client.sendError(
            interaction,
            `При поиске произошла ошибка: ${err.message}`
          );
        }
        switch (res.loadType) {
          case "NO_MATCHES":
