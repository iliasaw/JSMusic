...
module.exports = {
  name: "queue",
  description: "Показывает все песни, поставленные в очередь",
  usage: "",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!player.queue || !player.queue.length || player.queue === 0) {
      let QueueEmbed = new MessageEmbed()
        ...
        .addField("Запрошен:", `${player.queue.current.requester}`, true)
        .addField(
          "Продолжительность",
          `${
            client.ProgressBar(
    ...
    let Pages = ChunkedSongs.map((Tracks) => {
      ...
      let Embed = new MessageEmbed()
        ...
        .addField("Запрошен:", `${player.queue.current.requester}`, true)
        .addField(
          "Продолжительность текущей песни:",
          `${
            client.ProgressBar(
              player.position,
              player.queue.current.duration,
