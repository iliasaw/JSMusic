...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    SongTitle = SongTitle.replace(
      /lyrics|lyric|lyrical|official music video|\(official music video\)|audio|official|текст|текст песни|official video|official video hd|official hd video|offical video music|\(offical video music\)|extended|hd|(\[.+\])/gi,
      ""
    );
    ...
    if (!lyrics)
      return client.sendTime(
        message.channel,
        `**Текст песни не найден -** \`${SongTitle}\``
      );
    lyrics = lyrics.split("\n"); //spliting into lines
    ...
    let Pages = SplitedLyrics.map((ly) => {
      let em = new MessageEmbed()
        .setAuthor(`Текст для: ${SongTitle}`, client.botconfig.IconURL)
    ...
    options: [
      {
        ...
        value: "song",
        type: 3,
        description: "Введите название песни для поиска",
        required: false,
      },
    ],
    /**
